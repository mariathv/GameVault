...
const Inventory = () => {
    ...
    return (
      <div className="container mx-auto p-4">
        <div className="bg-(--color-secondary-background)/10 p-6 rounded-xl text-center text-(--color-foreground)">
          ...
          <p className="text-(--color-foreground)/80">
  ...
  return (
    <div className="min-h-screen bg-(--color-background)">
      <div className="container mx-auto px-4 py-8">
        ...
        {inventoryItems && inventoryItems.length === 0 ? (
          <div className="bg-(--color-secondary-background)/10 p-8 rounded-xl text-center">
            <h3 className="text-xl font-medium text-(--color-foreground) mb-2">
              Your library is empty
            </h3>
            <p className="text-(--color-foreground)/80 mb-4">
              Explore the store to discover and purchase new games for your
              collection.
            </p>
            ...
            {inventoryItems &&
              inventoryItems.map((order) => (
                <Card
                  ...
                  className="bg-(--color-secondary-background)/5 border-0 shadow-sm"
                >
                  <CardContent className="p-6">
                    ...
                    {order.games?.length > 0 ? (
                        ...
                        {order.games.map((game) => (
                          ...
                          >
                            ...
                            <div className="w-2/3 p-3">
                              <h4 className="font-medium mb-1 text-(--color-foreground) line-clamp-1">
                      ...
                      <p className="text-center text-(--color-foreground)/60 py-4">
                        No games in this order
                      </p>
                    )}
                  </CardContent>
                </Card>
      ...
      {modalOpen && (
        <GameDetailModal selectedGame={selectedGame} closeModal={closeModal} />
      )}
    </div>
  );
};
