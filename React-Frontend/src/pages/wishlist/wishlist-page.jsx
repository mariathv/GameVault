...
const dummyWishlistGames = [
  ...
  },
  {
    ...
    description:
      "Create and manage your own city without restrictions. Offering a deep simulation and a living economy, Cities: Skylines II will challenge your decision-making skills and creativity.",
  },
]
        ...
        {wishlistItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {wishlistItems.map((game) => (
              <div key={game.id} className="bg-[#1c2536] rounded-lg overflow-hidden">
                <div className="flex flex-col sm:flex-row">
                  <div className="sm:w-1/3">
                    <img
                      src={game.image || "/placeholder.svg"}
                      alt={game.name}
                      className="w-full h-full object-cover aspect-[3/4] sm:aspect-auto"
                    />
                  </div>
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="flex flex-col justify-between items-start gap-4">
                      ...
                      <div className="text-2xl font-bold text-white self-start">${game.price.toFixed(2)}</div>
                    </div>

                    <div className="mt-auto pt-4 flex flex-col sm:flex-row flex-wrap gap-3">
                      <button
                        onClick={() => handleAddToCart(game)}
                        className="px-4 py-2 bg-white text-black font-medium rounded-md hover:bg-gray-200 transition flex items-center justify-center"
                      >
                        Add to Cart
                      </button>
                      <button
