...
export default function GamePage() {
  ...
  const fetchGameData = async () => {
    try {
      ...
      const promises = [
        ...
        fetchPlatforms(gameData.platforms),
        fetchReleaseDates(gameData.release_dates),
      ];
  ...
  const fetchPlatforms = async (platformIds) => {
    ...
    console.log("palts", platforms);

    if (platforms.length > 2) {
        const shuffled = [...platforms].sort(() => 0.5 - Math.random());
        setPlatformDetails(shuffled.slice(0, 2));
        return shuffled.slice(0, 2);
    }

    setPlatformDetails(platforms);
    return platforms;
  };
  ...
  const fetchReleaseDates = async (releaseDateIds) => {
    ...
    const res = await fetchData(
      `games/get/release-dates?ids=${releaseDateIds}`
    );
    console.log("response data",res);
    setReleaseDates(res.queryResult);
    return res;
  };
  ...
  const handleWishlistAction = async () => {
    ...
    try {
      if (isInWishlist) {
        ...
        showNotification(
          `${game.name} has been removed from your wishlist.`,
          "success"
        );
      } else {
  ...
  const extractFeaturesFromGame = (game) => {
    ...
    if (game.game_modes?.includes(2) || game.multiplayer_modes?.length > 0)
    ...
    if (game.category === 0 || game.genres?.includes(31))
      features.push("Open World");
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-(--color-background) flex flex-col items-center justify-center text-(--color-foreground)">
        <div className="loader border-t-4 border-(--color-foreground)"></div>{" "}
        {/* Loader */}
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-(--color-background)">
      ...
      {artworks && (
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.6)), url(${createImageUrl(
              artworks[0]?.image_id
            )})`,
          }}
        >
          <div className="container mx-auto px-4 py-8 md:py-16 lg:py-24">
            <div className="flex flex-col md:flex-row md:gap-8 lg:gap-16 items-center">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="text-white/80 line-clamp-3 text-sm pt-2 max-w-2xl">
                  {game.storyline}
                </p>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
              ...
              <img
                src={
                  activeScreenshot === 0 && artworks && artworks.length > 0
                    ? createImageUrl(
                        artworks[1]?.image_id || artworks[0]?.image_id
                      )
                    : screenshots && screenshots.length > 0
                    ? createImageUrl(
                        screenshots[activeScreenshot - 1]?.image_id
                      )
              ...
              {screenshots &&
                ...
                screenshots?.slice(0, 4)?.map((screenshot, index) => (
                  ...
                  >
                    <img
                      src={
                        createImageUrl(screenshot.image_id) ||
                        "/placeholder.svg"
                      }
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4 text-(--color-foreground)">
              <Badge
                variant="outline"
                className="border-(--color-light-ed)/20 bg-[#EDEDED]/5"
              >
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between">
                ...
                <span className="text-(--color-light-ed) mt-1 sm:mt-0">
                  {Array.isArray(platformDetails)
                    ? platformDetails.map((p) => p.name).join(", ") || "N/A"
                    : "N/A"}
                </span>
              </div>
            </div>
          ...
          >
            <div className="overflow-x-auto">
              <TabsList className="bg-(--color-light-ed)/5 border-b border-(--color-light-ed)/10 text-(--color-light-ed) w-full">
                <TabsTrigger
                  value="overview"
                  className="data-[state=active]:bg-(--color-light-ed)/10"
                >
                  Overview
                </TabsTrigger>
            ...
            <TabsContent value="overview" className="pt-6">
              <p className="text-(--color-light-ed)/80 whitespace-pre-line">
                ...
                {gameFeatures && gameFeatures.length > 0 ? (
                  gameFeatures.map((feature) => (
                    <div key={feature} className="flex items-start gap-2">
                      ...
                      <span className="text-(--color-light-ed)/80">
                        {feature}
                      </span>
                    </div>
                  ))
                  ...
                  <p className="text-sm text-muted">
                    No specific features listed for this game.
                  </p>
                )}
              </div>
            </TabsContent>
