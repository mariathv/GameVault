...
export function GameModal({ isOpen, onClose, game, inStore }) {
  ...
  const handleSetFeatured = async () => {
    try {
      const response = await apiRequest("store/set-featured", {
        gameId: gameInfo._id,
      });
      if (response.success) {
        setIsFeatured(true);
  ...
  const downloadCSV = () => {
    ...
    const csvContent =
      "data:text/csv;charset=utf-8," + gameInfo.gameKeys.join("\n");
  ...
  const updateStoreGame = async () => {
    try {
      ...
      const response = await apiRequest("store/games/update/", {
        update: updatedGame,
      });
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        setGameInfo((prev) => {
          const existingKeys = Array.isArray(prev.gameKeys)
            ? prev.gameKeys
            : [];
          const mergedKeys = Array.from(
            new Set([...existingKeys, ...firstColumn])
          );
          return {
            ...prev,
            gameKeys: mergedKeys,
      ...
      <DialogContent className="bg-[#1D2127] text-white border-[#2D3237] max-w-[90vw] sm:max-w-[85vw] md:max-w-[75vw] lg:max-w-[65vw] xl:max-w-4xl p-6 sm:p-6 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">
            {inStore ? "Edit Game" : "Add Game to Store"}
          </DialogTitle>
        </DialogHeader>
            ...
            <Tabs defaultValue="details" className="w-full">
              ...
              >
                <div className="space-y-1 sm:space-y-2">
                  <Label className="font-bold text-base sm:text-lg line-clamp-2">
                    {gameInfo.name}
                  </Label>
                </div>
                ...
                {discountSetLoading && (
                  <div className="loader-dots ml-5 text-white"></div>
                )}
                {discountSet && (
                  <h3 className="text-xs text-green-600">
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-2">
                  ...
                  >
                    <Trash className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> Remove From
                    Store
                  </Button>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label className="text-sm sm:text-base">
                      Game Keys ({gameInfo.gameKeys?.length || 0})
                    </Label>
                  </div>
                    ...
                    {gameInfo.gameKeys && gameInfo.gameKeys.length > 0 ? (
                      <ul className="space-y-1">
                        {gameInfo.gameKeys.map((key, index) => (
                          <li
                            key={index}
                            className="text-xs sm:text-sm font-mono bg-[#3D4247] p-1 rounded"
                          >
                            {key}
                          </li>
                        ))}
