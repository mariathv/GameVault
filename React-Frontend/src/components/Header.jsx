                ...
                <div className="flex items-center justify-between">
                    ...
                    <Link to="/" className="text-xl sm:text-2xl font-bold text-(--color-foreground) mr-4 sm:mr-10 flex-shrink-0">
                        <div className="flex items-center">
                            <img src="/src/assets/imgs/logo/1.png" className="w-8 sm:w-10" alt="GameVault Logo" />
                    ...
                    <nav className="hidden md:flex items-center gap-8 text-(--color-foreground) pr-4" ref={dropdownRef}>
                                        ...
                                        {gameThemes.map((column, colIndex) => (
                                            <div key={colIndex} className="space-y-2">
                                                {column.map((theme, index) => {
                                                    const themeId = Object.keys(themes).find((key) => themes[key] === theme)
                        ...
                        {isAuthenticated ? (
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger className="flex items-center gap-1 text-sm text-(--color-foreground) h-8 px-2 sm:px-5 rounded bg-transparent hover:bg-(--color-foreground)/10 focus:outline-none">
                                    <Button variant="ghost" size="icon" className="text-(--color-foreground) p-0">
                    ...
                    <div className="md:hidden mt-4 pb-4 border-t border-(--color-foreground)/10 pt-4">
                        <nav className="flex flex-col space-y-4 text-(--color-foreground)">
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    <span>Themes</span>
                                    <ChevronDown className={`h-4 w-4 transition-transform ${mobileThemesOpen ? "rotate-180" : ""}`} />
                                </button>
                                {mobileThemesOpen && (
                                    <div className="pl-4 space-y-2 mt-2">
                            ...
                            {isAuthenticated ? (
                                ...
                                <Link to="/login" className="text-(--color-foreground)/80 hover:text-(--color-foreground) flex items-center">
