...
const authController = {
  register: catchAsync(async (req, res) => {
    try {
      ...
      const token = jwt.sign({ email, username, password, passwordConfirm, role }, process.env.JWT_SECRET, {
        expiresIn: "1d",
      })
    ...
    try {
      ...
      if (existingUser.twoFactorEnabled) {
        try {
          ...
          const tempToken = jwt.sign({ id: existingUser._id, require2FA: true }, process.env.JWT_SECRET, {
            expiresIn: "30m",
          })
